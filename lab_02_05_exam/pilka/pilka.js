...
class Colisions {
  ...
  collisionsChecker() {
    ...
    // vertical wall - ball changes horizontal direction - (x, -y)
    if (
      ...
      console.log(`ball hits vertical wall at ${nextI}, ${nextJ}`);
      newVec = {
        x: vec.x,
        y: -vec.y,
      };
    ...
    // horizontal wall - ball changes vertical direction - (-x, y)
    if (
      board[nextI][nextJ] == "X" ||
      ...
      console.log(`ball hits horizontal wall at ${nextI}, ${nextJ}`);
      newVec = {
        x: -vec.x,
        y: vec.y,
      };
    ...
    // Cross wall - ball changes horizontal direction - (-x, y)
    if (
      board[nextI][nextJ] == "X" ||
      (board[nextI][nextJ] == "XX" && (nextJ == 3 || nextJ == 5) && nextI == 10)
    ) {
      console.log(`ball hits Cross at ${nextI}, ${nextJ}`);
      newVec = {
        x: vec.x,
        y: -vec.y,
      };
    ...
    // Cross wall - ball changes vertical direction - (x, -y)
    if (
      board[nextI][nextJ] == "X" ||
    ...
    // corners and places next to corners - ball baunces back - (-x,-y)
    else if (board[nextI][nextJ] == "X" || board[nextI][nextJ] == "XX") {
      console.log(`ball hits corner or wall near corner at ${nextI}, ${nextJ}`);
      newVec = {
        x: -vec.x,
        y: -vec.y,
      };
    ...
    // Y - ball bounces in random direction - (-x,y) or (x,-y)
      ...
      // normal movement without obstacle
    } else {
      newVec = {
        x: vec.x,
        y: vec.y,
